...
- 支持可配置的统一面板快捷键，默认 `Cmd+F1`；按一次切换 Codex 面板，400ms 内连按两次切换 Xcode Dashboard。
- 提供完全独立的 Xcode Dashboard，展示当前项目窗口列表，点击行即可精准唤醒对应窗口。
...
- 默认 `Cmd+F1`：按一次打开或关闭 Codex 面板，400ms 内连按两次打开或关闭独立 Xcode Dashboard。
- `↑` / `↓`：在展开面板中切换选中会话。
- `Return`：进入当前选中会话。
- `Esc`：收起展开面板。
...
- 默认 `Cmd+F1` 单次触发能切换 Codex 面板，400ms 内双次触发能切换独立 Xcode Dashboard；列表只展示项目窗口，点击任意一行后对应 Xcode 窗口被提升到前台。
- Codex Dashboard 与 Xcode Dashboard 保持互斥；切换到另一面板时会先收起当前面板。
