...
打包脚本会同时复制 SwiftPM 资源 bundle，其中包含从本机 Codex 自定义宠物升级得到的 Trump v2 宠物包；运行时不再依赖 `~/.codex/pets`。
...
- 悬浮入口使用 App 内置的 Trump Codex v2 宠物包：普通状态播放 idle，任务运行播放专注处理动作，需要输入或疑似停滞时播放等待动作，失败时播放受挫动作；不绘制圆形底、光环或独立状态点。
...
- Trump v2 图集从本机已安装的 Codex Trump 自定义宠物升级而来：保留原有标准动作，修复 v2 动作语义并补齐 16 个观察方向；运行时只读取 App 内置资源。
